...
def join_room(request, sockets, socket, channels, message_queues):
    ...
    response = buildMessage(const.JOIN_SUCCESS_CMD, request['nickname'], room, response_text)
    message_queues[socket_id].put(response)

    notificationText = request['nickname'] + " has joined room " + room
    notificationMessage = buildMessage(const.BROADCAST_CMD, '', '', notificationText)

    for key, s in sockets.items():
        socket_id = s['socketConnection'].fileno()
        if room in sockets[socket_id]['rooms'] and s['socketConnection'] != socket:
            message_queues[socket_id].put(notificationMessage)
...
def leave_room(request, sockets, socket, rooms, message_queues):
    ...
    notificationText = request['nickname'] + " has left room " + room
    notificationMessage = buildMessage(const.BROADCAST_CMD, '', '', notificationText)
...
def set_nickname(socket, request, sockets, message_queues):
    ...
    for key, s in sockets.items():
        if s['nickname'] == nickname:
            message = {'nickname': nickname, 'command': const.NICK_FAIL_CMD, 'channel': '', 'message': 'Nickname ' + nickname+  ' is taken.'}
            break
        else:
...
def send_direct_message(request, sockets, s, message_queues, outputs):
    ...
    for key, socket in sockets.items():
        ...
        if socket['nickname'] == destination:
            message_queues[socket['socketId']].put(buildMessage(const.DIR_MSG_COMD, request['nickname'], 'direct',
                                                                      request['message']))
